...
def test_generate_plan(mock_config, mock_args):
    ...
    test_tool = SystemTool(
        ...
        parameters=[
            Parameter(name="input", type="str", description="Input value")
        ],
    ...
    with patch('evolia.integrations.openai_structured.call_openai_structured') as mock_openai:
...
def test_generate_plan_error(mock_config, mock_args):
    ...
    with patch('evolia.core.evolia.call_openai_structured', side_effect=RuntimeError("API error")):
