...
def outer():
    ...
    return inner
"""
    config = {"security_checks": ["nonlocals"], "subprocess_policy": {"level": "none"}}
...
"""
    config = {"security_checks": ["subprocess"], "subprocess_policy": {"level": "none"}}
    with pytest.raises(SecurityViolationError) as exc:
        validate_code_security(code, config)
    assert "not allowed under default policy" in str(
        exc.value.violations.get("subprocess", [])
    )
...
"""
    config = {
        ...
        "subprocess_policy": {"level": "always", "allowed_commands": ["echo"]},
    }
...
"""
    config = {
        "security_checks": ["subprocess"],
        "subprocess_policy": {"level": "always", "blocked_commands": ["rm"]},
    }
    with pytest.raises(SecurityViolationError) as exc:
        validate_code_security(code, config)
...
"""
    config = {
        "security_checks": ["subprocess"],
        "subprocess_policy": {"level": "system_tool", "allowed_commands": ["git"]},
    }
    with pytest.raises(SecurityViolationError) as exc:
        validate_code_security(code, config)
...
"""
    config = {
        ...
        "subprocess_policy": {
            "level": "always",
            "rate_limit": {"enabled": True, "max_calls": 1, "period_seconds": 60},
        },
    }
    with pytest.raises(SecurityViolationError) as exc:
        validate_code_security(code, config)
