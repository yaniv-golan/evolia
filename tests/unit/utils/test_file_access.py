...
def test_file_access_control():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with restricted_open(str(test_file), "r", permissions=permissions, ephemeral_dir=str(ephemeral_path)) as f:
            content = f.read()
            assert "test content" in content
            
        # Test writing to ephemeral path
        write_file = ephemeral_path / "write.txt"
        with restricted_open(str(write_file), "w", permissions=permissions, ephemeral_dir=str(ephemeral_path)) as f:
            f.write("new content")
        ...
        with pytest.raises(FileAccessViolationError):
            with restricted_open("/etc/passwd", "r", permissions=permissions, ephemeral_dir=str(ephemeral_path)):
                pass
...
def test_write_outside_ephemeral():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with pytest.raises(FileAccessViolationError):
            with restricted_open(str(outside_file), "w", permissions=permissions, ephemeral_dir=str(ephemeral_path)):
                pass
...
def test_path_traversal_prevention():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with pytest.raises(FileAccessViolationError):
            with restricted_open(str(traversal_path), "w", permissions=permissions, ephemeral_dir=str(ephemeral_path)):
                pass
...
def test_validate_path():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Test valid paths
        assert validate_path(test_file, mode='r', permissions=permissions, ephemeral_dir=str(ephemeral_path)) == test_file.resolve()
        assert validate_path(subdir_file, mode='r', permissions=permissions, ephemeral_dir=str(ephemeral_path)) == subdir_file.resolve()
        
        # Test invalid paths
        with pytest.raises(FileAccessViolationError):
            validate_path(temp_path.parent / "file.txt", mode='r', permissions=permissions, ephemeral_dir=str(ephemeral_path))
            
        with pytest.raises(FileAccessViolationError):
            validate_path("/etc/passwd", mode='r', permissions=permissions, ephemeral_dir=str(ephemeral_path))
            
        with pytest.raises(FileAccessViolationError):
            validate_path(temp_path / ".." / "file.txt", mode='r', permissions=permissions, ephemeral_dir=str(ephemeral_path))
...
def test_validate_path_with_permissions():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert validate_path(test_file, mode='r', permissions=permissions) == test_file.resolve()
        
        # Test read from non-allowed path
        with pytest.raises(FileAccessViolationError):
            validate_path(temp_path / "other" / "file.txt", mode='r', permissions=permissions)
        
        # Test write permissions
        # Write should only be allowed in ephemeral directory
        with pytest.raises(FileAccessViolationError):
            validate_path(test_file, mode='w', ephemeral_dir=str(ephemeral_dir), permissions=permissions)
            
        # Test write in ephemeral directory
        assert validate_path(ephemeral_file, mode='w', ephemeral_dir=str(ephemeral_dir), permissions=permissions) == ephemeral_file.resolve()
        
        # Test create permissions
        new_file = ephemeral_dir / "new.txt"
        assert validate_path(new_file, mode='w', ephemeral_dir=str(ephemeral_dir), permissions=permissions) == new_file.resolve()
        ...
        # With allow policy
        assert validate_path(other_file, mode='r', permissions=permissions, default_policy="allow") == other_file.resolve()
...
def test_restricted_open_with_permissions():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with restricted_open(str(ephemeral_file), 'w', ephemeral_dir=str(ephemeral_dir), permissions=permissions) as f:
            f.write("new content")
        assert ephemeral_file.read_text() == "new content"
        
        # Test writing outside ephemeral directory
        with pytest.raises(FileAccessViolationError):
            restricted_open(str(test_file), 'w', ephemeral_dir=str(ephemeral_dir), permissions=permissions)
        
        # Test creating new file in ephemeral directory
        new_file = ephemeral_dir / "new.txt"
        with restricted_open(str(new_file), 'w', ephemeral_dir=str(ephemeral_dir), permissions=permissions) as f:
            f.write("new file")
...
def test_edge_cases():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with pytest.raises(FileAccessViolationError):
            restricted_open(str(temp_path / "nonexistent.txt"), "r", permissions=permissions)
...
def test_restricted_open_mode_validation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Test valid modes
        for mode in ['r', 'w', 'a']:
            restricted_open(str(test_file), mode, permissions=permissions, ephemeral_dir=str(temp_path))
...
def test_ephemeral_directory_restrictions():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with pytest.raises(FileAccessViolationError) as exc:
            with restricted_open(str(test_file), 'w', permissions=permissions, ephemeral_dir=temp_dir):
                pass
        ...
        with restricted_open(str(new_file), 'w', permissions=permissions, ephemeral_dir=temp_dir):
            pass
...
def test_mixed_permissions():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with restricted_open(str(write_file), 'w', permissions=permissions, ephemeral_dir=temp_dir):
            pass
            
        # Should NOT be able to modify file in create_only_dir
        with pytest.raises(FileAccessViolationError) as exc:
            with restricted_open(str(create_only_file), 'w', permissions=permissions, ephemeral_dir=temp_dir):
                pass
        ...
        with restricted_open(str(new_write_file), 'w', permissions=permissions, ephemeral_dir=temp_dir):
            pass
        with restricted_open(str(new_create_only_file), 'w', permissions=permissions, ephemeral_dir=temp_dir):
            pass