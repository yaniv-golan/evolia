...
def wait_for_endpoint(endpoint_name, timeout=300, check_interval=10):
    ...
    while time.time() - start_time < timeout:
        ...
        if status == "running":
            ...
            print(
                f"Endpoint {endpoint_name} is {status}, waiting {check_interval} seconds..."
            )
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    and "is currently loading" in response_json["error"]
                ):
                    estimated_time = response_json.get("estimated_time", retry_delay)
                    print(
                        f"Model is loading. Estimated time: {estimated_time:.1f} seconds"
                    )
...
def test_plan_generation():
    ...
    if status != "running":
        pytest.skip(
            f"Endpoint {full_name} is not running (status: {status}). Please use resume_endpoints.py to start the endpoints first."
        )
