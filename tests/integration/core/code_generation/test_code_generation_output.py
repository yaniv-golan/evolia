...
from evolia.utils.exceptions import CodeGenerationError, APIRateLimitError, TemplateError
...
def test_successful_generation(code_generator, mock_responses):
    ...
    with patch('evolia.core.code_generator.call_openai_structured', return_value=mock_responses["success"]):
        response = code_generator.generate(
            prompt_template="Generate a function to {description}",
            template_vars={
                "description": "Process input data",
                "parameters": [Parameter(name="data", type="list", description="Input data")],
                "return_type": "dict"
            },
...
def test_invalid_syntax_output(code_generator, mock_responses):
    ...
    with patch('evolia.core.code_generator.call_openai_structured', return_value=mock_responses["invalid_syntax"]):
...
def test_security_issue_output(code_generator, mock_responses):
    ...
    with patch('evolia.core.code_generator.call_openai_structured', return_value=mock_responses["security_issue"]):
        with pytest.raises(CodeGenerationError) as exc_info:
            code_generator.generate(
...
def test_edge_cases(code_generator):
    ...
    test_cases = [
        ...
        ({"code": "def func():\n    pass", "validation_results": None}, "missing validation")
    ]
    
    for response, expected_error in test_cases:
        with patch('evolia.core.code_generator.call_openai_structured', return_value=response):
            with pytest.raises(CodeGenerationError) as exc_info:
                code_generator.generate(
...
def test_invalid_type_output(code_generator, mock_responses):
    ...
    with patch('evolia.core.code_generator.call_openai_structured', return_value=invalid_response):
        with pytest.raises(CodeGenerationError) as exc_info:
            code_generator.generate(
                prompt_template="Generate a function",
                template_vars={
                    "description": "Process data",
                    "parameters": [Parameter(name="data", type="list", description="Input")],
                    "return_type": "dict"
                },
                schema={"type": "object", "properties": {}}
            )
...
def test_rate_limit_handling(code_generator):
    ...
    with patch('evolia.core.code_generator.call_openai_structured', side_effect=raise_rate_limit):
...
def test_api_error_handling(code_generator):
    ...
    for error_msg, expected_phrase in error_cases:
        ...
        with patch('evolia.core.code_generator.call_openai_structured', side_effect=raise_api_error):
            with pytest.raises(CodeGenerationError) as exc_info:
                code_generator.generate(
                    prompt_template="Generate a function",
                    template_vars={},
