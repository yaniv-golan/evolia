...
def call_openai_structured(
    ...
    logger.info("Starting OpenAI API call", extra={
        'payload': {
            'component': 'openai',
    ...
    for attempt in range(max_retries):
        try:
            logger.debug(f"Attempt {attempt + 1}/{max_retries}", extra={
                'payload': {
                    'component': 'openai',
                    'operation': 'structured_call',
                    'attempt': attempt + 1,
            ...
            # Log request payload
            logger.debug("OpenAI API request", extra={
                'payload': {
                    'component': 'openai',
                    'operation': 'structured_call',
                    'request': {
            ...
            logger.debug("Received OpenAI API response", extra={
                'payload': {
                    'component': 'openai',
                    'operation': 'structured_call',
                    'attempt': attempt + 1,
            ...
            if not content:
                logger.error(ERROR_MESSAGES['empty_response'], extra={
                    'payload': {
                        'component': 'openai',
                        'operation': 'structured_call',
                        'response_id': response.id
            ...
            try:
                ...
                logger.info("Successfully parsed OpenAI response", extra={
                    'payload': {
                        'component': 'openai',
                        'operation': 'structured_call',
                        'response_id': response.id
                ...
                logger.error(error_msg, extra={
                    'payload': {
                        ...
                        'operation': 'structured_call',
                        'error': str(e),
                        'content': content[:1000]  # Log first 1000 chars of invalid JSON
                    }
                })
    ...
    raise RuntimeError(ERROR_MESSAGES['max_retries'].format(
        max_retries=max_retries,
        error="Unknown error"
    ))