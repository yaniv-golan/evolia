...
class LoggedRequests:
    ...
    def validate_url(self, url: str):
        ...
        # Check HTTP/HTTPS
        if (
            not self.config.get("access", {}).get("allow_http", True)
            and parsed.scheme == "http"
        ):
            logger.warning(
                WARNING_MESSAGES["http_warning"].format(url=url),
                extra={
                    "payload": {
                        "url": url,
                        "component": "network",
        ...
        if not self.config.get("access", {}).get("allow_external", True):
            if parsed.netloc not in self.whitelist_domains:
                logger.warning(
                    WARNING_MESSAGES["external_warning"].format(url=url),
                    extra={
                        "payload": {
                            "url": url,
                            "component": "network",
        ...
        if not self.is_whitelisted(url):
            logger.warning(
                WARNING_MESSAGES["whitelist_warning"].format(url=url),
                extra={
                    "payload": {
                        "url": url,
                        "component": "network",
            ...
            raise PermissionError(
                ERROR_MESSAGES["domain_not_whitelisted"].format(url=url)
            )
    ...
    def log_request(self, method: str, url: str, **kwargs):
        ...
        logger.info(f"Making {method} request to {url}", extra={"payload": log_data})
    ...
    def _make_request(self, method: str, url: str, **kwargs):
        ...
        if self.rate_limiter is not None:
            if not self.rate_limiter.check_rate_limit():
                logger.warning(
                    "Rate limit exceeded",
                    extra={"payload": self._extract_log_data(method, url, **kwargs)},
                )
                raise Exception(
                    f"Rate limit exceeded: {self.rate_limiter.max_calls} calls per {self.rate_limiter.period} seconds"
                )
