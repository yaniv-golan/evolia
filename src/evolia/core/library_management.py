...
class ImportValidator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.blocked_modules = set(config.get('library_management', {}).get('blocked_libraries', []))
    ...
    def validate_imports(self, imports: Set[str]) -> List[str]:
        ...
        for imp in imports:
            ...
            if imp in self.allowed_modules and self.allowed_modules[imp]:
                valid, version = validate_library_version(imp, self.allowed_modules[imp])
                if not valid:
                    errors.append(f"Module {imp} version {version} does not meet minimum requirement {self.allowed_modules[imp]}")
...
STDLIB_MODULES = {
    ...
    'difflib', 'enum', 'fileinput', 'fnmatch', 'functools', 'glob', 'hashlib',
    'heapq', 'hmac', 'html', 'http', 'importlib', 'inspect', 'io', 'itertools',
    'json', 'logging', 'math', 'multiprocessing', 'operator', 'os', 'pathlib',
    'pickle', 'platform', 'pprint', 'queue', 're', 'shutil', 'signal', 'socket',
    'sqlite3', 'statistics', 'string', 'subprocess', 'sys', 'tempfile', 'threading',
...
def detect_missing_libraries(required_libraries: List[str]) -> List[str]:
    ...
    for lib in required_libraries:
        ...
        try:
            ...
            missing.append(lib)
            logger.warning(f"Library {lib} not found", extra={
                'payload': {
                    'library': lib,
...
def validate_library_version(library: str, min_version: Optional[str] = None) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        logger.debug(f"Checking version for {library}", extra={
            'payload': {
                'library': library,
        ...
        if min_version:
            try:
                    ...
                    extra={
                        'payload': {
                            ...
                            'validation_details': {
                                ...
                                'comparison_result': 'version_too_low' if not is_valid else 'version_ok'
                            }
                        }
                    }
                )
                ...
                logger.error(f"Invalid version format for {library}", extra={
                    'payload': {
                        'library': library,
                        'installed_version': installed_version,
                        'minimum_version': min_version,
        ...
        logger.debug(f"No version requirement for {library}", extra={
            'payload': {
                'library': library,
                'installed_version': installed_version,
                'component': 'library_management',
        ...
        logger.error(f"Package {library} not found", extra={
            'payload': {
                'library': library,
    ...
    try:
        ...
        for req in requires:
            ...
            try:
                ...
                logger.debug(f"Found dependency for {library}", extra={
                    'payload': {
                        'library': library,
                ...
                logger.warning(f"Failed to parse requirement '{req}' for {library}", extra={
                    'payload': {
                        'library': library,
                        'requirement': req,
                        'error': str(e),
                ...
                try:
                    ...
                    simple_name = req.split(' ')[0].split('>=')[0].split('<=')[0].split('==')[0]
                    dependencies.add(simple_name)
                    logger.debug(f"Extracted simple name for invalid requirement", extra={
                        'payload': {
                            'library': library,
                            'requirement': req,
                    ...
                    logger.error(f"Failed to extract simple name from requirement '{req}'", extra={
                        'payload': {
                            'library': library,
                            'requirement': req,
                            'error': str(e2),
        ...
        logger.warning(f"Package {library} not found", extra={
            'payload': {
                'library': library,
                'component': 'library_management'
            }
        ...
        logger.error(f"Error getting dependencies for {library}: {str(e)}", extra={
            'payload': {
                'library': library,
                'error': str(e),
                'component': 'library_management'
...
class LibraryManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        runtime_overrides = config.get('library_management', {}).get('runtime_overrides', {})
        ...
        logger.debug("Library manager initialized", extra={
            'payload': {
    ...
    def check_dependencies(self, libraries: Set[str]) -> List[str]:
        ...
        for lib in libraries:
            ...
            logger.debug(f"Found dependencies for {lib}", extra={
                'payload': {
                    'library': lib,
        ...
        if disallowed:
            logger.warning("Found disallowed dependencies", extra={
                'payload': {
    ...
    def add_allowed_library(self, library: str, min_version: Optional[str] = None) -> None:
        ...
        logger.info(f"Added library {library} to allowed list", extra={
            'payload': {
                'library': library,
