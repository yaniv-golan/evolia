...
def verify_interface(
    interface: FunctionInterface, generated_code: Dict[str, Any]
) -> List[str]:
    ...
    if function_info["name"] != interface.function_name:
        errors.append(
            f"Function name mismatch: expected {interface.function_name}, got {function_info['name']}"
        )
    ...
    if interface_params != generated_params:
        errors.append(
            f"Parameter mismatch: expected {interface_params}, got {generated_params}"
        )

    # Check return type matches
    if function_info["return_type"] != interface.return_type:
        errors.append(
            f"Return type mismatch: expected {interface.return_type}, got {function_info['return_type']}"
        )
...
def match_example(
    generated_example: Dict[str, Any], interface_example: Dict[str, Any]
) -> bool:
    ...
    try:
        # Check inputs match
        if not match_dict_structure(
            generated_example.get("inputs", {}), interface_example.get("inputs", {})
        ):
            return False

        # Check expected outputs match
        if not match_dict_structure(
            generated_example.get("expected", {}), interface_example.get("expected", {})
        ):
            return False
