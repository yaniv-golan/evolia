...
from ..utils.logger import setup_logger, code_generation_context, validation_context, execution_context
    ...
    "properties": {
        "steps": {
            ...
            "items": {
                                ...
                                "items": {
                                    ...
                                    "properties": {
                                        ...
                                        "type": {
                                            ...
                                            "enum": [
                                                "Tuple", "Union", "bool", "float", "Any",
                                                "Dict", "str", "int", "set", "dict", "list",
                                                "tuple", "Set", "Optional", "List"
                                            ]
                                        },
                                        "description": {
                                            "type": "string",
                ...
                "required": [
                    "name", "tool", "inputs", "outputs",
                    "allowed_read_paths", "allowed_write_paths",
                    "allowed_create_paths", "default_policy"
                ],
                "additionalProperties": False
            },
            "minItems": 1
...
def validate_step_interface(step: PlanStep, system_tools: Dict[str, SystemTool]) -> StepValidationBase:
...
def execute_plan(plan: Plan, system_tools: Dict[str, SystemTool], config: Dict[str, Any], args: Any) -> None:
    ...
    with execution_context("plan"):
        try:
            ...
            raise PlanExecutionError(f"Failed to execute step {i + 1}: {step.name}", step.name, {"error": str(e)})
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evolia: AI-powered task orchestration that evolves with your workflow.")
    ...
    file_access_group.add_argument(
        '--allow-read',
        nargs='+',
        help='Paths to allow reading from',
        default=[]
    )
    file_access_group.add_argument(
        '--allow-write',
        nargs='+',
        help='Paths to allow writing to',
        default=[]
    )
    file_access_group.add_argument(
        '--allow-create',
        nargs='+',
        help='Paths to allow creating files in',
        default=[]
    )
    file_access_group.add_argument(
        '--default-policy',
    ...
    ecs_group.add_argument("--tag-candidate", 
                          help="Move ephemeral code to candidates")
    ecs_group.add_argument("--promote-candidate", 
                          help="Promote a candidate to system tool")
    ecs_group.add_argument("--auto-promote", action="store_true",
                          help="When tagging, mark candidate for auto-promotion")
    ecs_group.add_argument("--description", 
                          help="Description for promoted tool")
    
    # Library management arguments
    lib_group = parser.add_argument_group('Library Management')
    lib_group.add_argument("--allow-lib", action="append", 
                       help="Allow a library at runtime (format: name[==version])")
    lib_group.add_argument("--block-lib", action="append",
                       help="Block a library at runtime")
    lib_group.add_argument("--check-deps", action="store_true",
                       help="Check dependencies of allowed libraries")
    lib_group.add_argument("--prompt-missing", action="store_true",
                       help="Prompt for missing libraries")
    lib_group.add_argument("--no-runtime-libs", action="store_true",
    ...
    logger.debug("Environment information", extra={
        'payload': {
    ...
    logger.debug("Command line arguments", extra={
    ...
    try:
        ...
        logger.debug("Updated file access configuration", extra={
            'payload': {
        ...
        logger.debug("Plan generation completed", extra={
            'payload': {
        ...
        # Execute plan
        ephemeral_dir = config.get('file_access', {}).get('ephemeral_dir', 'run_artifacts')
        executor = Executor2(config=config, keep_artifacts=args.keep_artifacts, ephemeral_dir=ephemeral_dir)
        ...
        logger.error(f"Code generation/validation error: {str(e)}", extra={
            'payload': {
                'code': getattr(e, 'code', ''),
        ...
        logger.error(f"Security violation: {str(e)}", extra={
            'payload': {
        ...
        logger.error(f"Execution error: {str(e)}", extra={
            'payload': {
