            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        self.logger.warning(
                            f"Attempt {attempt + 1} failed, retrying in {sleep_time:.1f}s: {str(e)}"
                        )
...
class ExecutorErrorHandler:
    ...
    def handle_validation_error(
        self, e: Exception, script: str, context: str, details: Optional[Dict] = None
    ) -> None:
...
class Executor2:
    ...
    def __init__(
        ...
        ephemeral_base = (
            config.get("file_access", {})
            .get("paths", {})
            .get("ephemeral_base", "run_artifacts")
        )
        ...
        self.allowed_modules = set(
            allowed_modules_cfg.keys()
            if isinstance(allowed_modules_cfg, dict)
            else allowed_modules_cfg
        )
        ...
        if not api_key:
            raise ValueError(
                f"Missing OpenAI API key in environment variable {openai_cfg.get('api_key_env_var')}"
            )
    ...
    def _load_system_tools(self) -> Dict[str, Dict]:
        ...
        try:
            with open(tools_file) as f:
                ...
                for tool_data in tools_list:
                    try:
                        ...
                        if "version" not in tool_data:
                            self.logger.warning(
                                f"Tool {tool_data.get('name', 'unknown')} missing version"
                            )
                            continue
                        ...
                        missing_fields = [
                            f for f in required_fields if f not in tool_data
                        ]
                        if missing_fields:
                            self.logger.error(
                                f"Tool {tool_data.get('name', 'unknown')} missing required fields: {missing_fields}"
                            )
                            continue

                        # Validate interface definition
                        if not self._validate_tool_interface_definition(
                            tool_data["interface"]
                        ):
                            self.logger.error(
                                f"Tool {tool_data.get('name')} has invalid interface definition"
                            )
                            continue
                        ...
                        self.logger.error(
                            f"Failed to validate tool {tool_data.get('name', 'unknown')}: {str(e)}"
                        )
                        continue
                ...
                self.logger.debug(
                    f"Loaded {len(tools_dict)} system tools",
                    extra={
                        "payload": {
    ...
    def _update_tool_metadata(
        ...
        self.logger.debug(
            f"Updated tool metadata for {tool_name}",
            extra={
                "payload": {
    ...
    def _validate_tool_metadata(self, tool: SystemTool) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f"Tool {tool.name} has invalid version numbers: {tool.version}"
                )
                return False
            ...
            if not tool.interface.function_name.isidentifier():
                self.logger.error(
                    f"Tool {tool.name} has invalid function name: {tool.interface.function_name}"
                )
                return False
            ...
            for param in tool.interface.parameters:
                if not param.name.isidentifier():
                    self.logger.error(
                        f"Tool {tool.name} has invalid parameter name: {param.name}"
                    )
                    return False
                if not param.type in ["str", "int", "float", "bool", "list", "dict"]:
                    self.logger.error(
                        f"Tool {tool.name} has invalid parameter type: {param.type}"
                    )
                    return False
                if not param.description:
                    self.logger.warning(
                        f"Tool {tool.name} parameter {param.name} missing description"
                    )

            # Validate return type
            if tool.interface.return_type != "dict":
                self.logger.error(
                    f"Tool {tool.name} has invalid return type: {tool.interface.return_type}"
                )
                return False
            ...
            for field in required_fields:
                if not getattr(tool, field, None):
                    self.logger.error(
                        f"Tool {tool.name} missing required field: {field}"
                    )
                    return False
            ...
            if tool.interface.examples:
                for i, example in enumerate(tool.interface.examples):
                    if (
                        ...
                        or "inputs" not in example
                        or "expected" not in example
                    ):
                        self.logger.error(
                            f"Tool {tool.name} has invalid example format at index {i}"
                        )
                        return False
            ...
            if tool.interface.constraints:
                if not isinstance(tool.interface.constraints, list):
                    self.logger.error(
                        f"Tool {tool.name} has invalid constraints format"
                    )
                    return False
            ...
            for input_name in tool.inputs:
                if not input_name.isidentifier():
                    self.logger.error(
                        f"Tool {tool.name} has invalid input name: {input_name}"
                    )
                    return False
            ...
            for output_name in tool.outputs:
                if not output_name.isidentifier():
                    self.logger.error(
                        f"Tool {tool.name} has invalid output name: {output_name}"
                    )
                    return False
            ...
            if hasattr(tool, "filepath") and tool.filepath:
                if not os.path.exists(tool.filepath):
                    self.logger.error(
                        f"Tool {tool.name} has invalid filepath: {tool.filepath}"
                    )
                    return False
    ...
    def _execute_system_tool(
        ...
        try:
            ...
            if not tool:
                ...
                tool_path = (
                    self.config.get("file_access", {})
                    .get("paths", {})
                    .get("tools_base", "tools/system")
                )
                ...
                if hasattr(tool_module, "execute"):
                    ...
                    raise ExecutorError(
                        f"Invalid system tool: {step.tool} (missing execute function)"
                    )
            ...
            if step.outputs:
                for name in step.outputs:
                    if name not in result:
                        raise ExecutorError(
                            f"Expected output '{name}' not found in tool result"
                        )
    ...
    def _validate_tool_version(self, tool: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            min_version = tuple(
                map(int, self.config.get("min_tool_version", "0.0.0").split("."))
            )
    ...
    def _validate_tool_module(self, module: Any, interface: FunctionInterface) -> bool:
        ...
        try:
            ...
            if len(param_names) != len(interface.parameters):
                self.logger.error(
                    f"Parameter count mismatch: expected {len(interface.parameters)}, got {len(param_names)}"
                )
                return False
            ...
            if sig.return_annotation != dict:
                self.logger.error(
                    f"Return type mismatch: expected dict, got {sig.return_annotation}"
                )
                return False
    ...
    def _validate_tool_outputs(
        self, outputs: Dict[str, Any], interface: FunctionInterface
    ) -> bool:
        ...
        try:
            ...
            for output in interface.outputs:
                ...
                if expected_type == "str" and not isinstance(value, str):
                    self.logger.error(
                        f"Output {output.name} type mismatch: expected str, got {type(value)}"
                    )
                    return False
                    ...
                    if has_numpy:
                        if not isinstance(value, (int, np.integer)):
                            self.logger.error(
                                f"Output {output.name} type mismatch: expected int, got {type(value)}"
                            )
                            return False
                    else:
                    ...
                    if has_numpy:
                        if not isinstance(value, (int, float, np.integer, np.floating)):
                            self.logger.error(
                                f"Output {output.name} type mismatch: expected float, got {type(value)}"
                            )
                            return False
                    else:
                    ...
                    self.logger.error(
                        f"Output {output.name} type mismatch: expected bool, got {type(value)}"
                    )
                    return False
                elif expected_type == "list" and not isinstance(
                    value, (list, tuple, np.ndarray if has_numpy else list)
                ):
                    self.logger.error(
                        f"Output {output.name} type mismatch: expected list, got {type(value)}"
                    )
                    return False
                elif expected_type == "dict" and not isinstance(value, dict):
                    self.logger.error(
                        f"Output {output.name} type mismatch: expected dict, got {type(value)}"
                    )
                    return False
    ...
    def _prepare_tool_globals(
        self, inputs: Dict[str, Any], step_dir: Path
    ) -> Dict[str, Any]:
    ...
    def cleanup(self, error_occurred: bool = False):
        ...
        if error_occurred:
            self.logger.info(
                f"Saving error state for debugging in: {self.artifacts_dir}"
            )
            try:
                # Save error state
    ...
    def resolve_reference(self, ref: Any) -> Any:
        ...
        self.logger.debug(
            f"Resolved reference {ref} to value",
            extra={
        ...
        try:
            ...
            return CodeGenerationResponse(
                ...
                parameters=[
                    ...
                    for p in response["function_info"]["parameters"]
                ],
                return_type=return_type,
    ...
    def _fix_code_with_retries(self, code: str, error_msg: str) -> str:
        ...
        try:
            ...
            logger.debug(
                "Code fix attempt details",
                extra={"payload": {"error_msg": error_msg, "code_length": len(code)}},
            )
    ...
    def _handle_runtime_error(
        ...
        try:
            ...
            result = self.restricted_executor.execute_in_sandbox(
                script=fixed_code, inputs=resolved_inputs, output_dir=str(step_dir)
            )

            # Extract outputs
            outputs = {
                name: result.get(var_name) for name, var_name in step.outputs.items()
            }
            logger.debug(
                "Successfully executed fixed code",
                extra={
                    "payload": {
    ...
    def execute_code(self, step: PlanStep, step_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.restricted_executor.execute_in_sandbox(
                script=code, inputs=inputs, output_dir=str(workspace_root / step_dir)
            )
            ...
            self.logger.error(error_msg)
            raise ExecutorError(
                error_msg, code if "code" in locals() else None, {"error": str(e)}
            )
            ...
            for output_name, output_def in step.outputs.items():
                ...
                if ref in output_refs or ref_curly in output_refs:
                    raise PlanValidationError(
                        f"Duplicate output reference found: {ref}"
                    )
                output_refs.add(ref)
            ...
            if hasattr(step, "depends_on"):
                for dep in step.depends_on:
                    ...
                    if step_indices[dep] >= step_indices[step.name]:
                        raise PlanValidationError(
                            f"Invalid dependency order: {step.name} depends on {dep}"
                        )
        ...
        for i, step in enumerate(plan.steps):
            ...
            for input_name, input_value in step.inputs.items():
                if isinstance(input_value, str) and (
                    "${" in input_value or input_value.startswith("$")
                ):
                    # Extract the step name from the reference
                    if "${" in input_value:
                        # Handle ${step.output} format
                        ref_step = input_value.split("${")[1].split(".")[
                            0
                        ]  # Extract step name from ${step.output}
                    else:
                        # Handle $step.output format
                        ref_step = input_value.split(".")[0][
                            1:
                        ]  # Remove $ and get step name

                    if ref_step not in step_indices:
                        raise PlanValidationError(
                            f"Step {step.name} references unknown step: {ref_step}"
                        )
                    if step_indices[ref_step] >= i:
                        raise PlanValidationError(
                            f"Invalid data flow: {step.name} references output from step {ref_step} which hasn't been executed yet"
                        )
                    if (
                        input_value not in output_refs
                        and input_value.replace("${", "$") not in output_refs
                    ):
                        raise PlanValidationError(
                            f"Step {step.name} references unknown output: {input_value}"
                        )
    ...
    def _attempt_runtime_fix(
        ...
        """
        return self._handle_runtime_error(
            script, error, restricted_globals, resolved_inputs, step_dir, step
        )
        ...
        with validation_context({"code": script, "step": step.name}) as val_ctx:
            ...
            if not validation_result.is_valid:
                raise CodeValidationError(
                    "Code validation failed", validation_result.details
                )
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.Call):
                    if isinstance(node.func, ast.Name) and node.func.id == "open":
                        if len(node.args) < 1:
                            raise SecurityViolationError(
                                "File operations must specify a file path"
                            )
            ...
            has_try_except = any(isinstance(n, ast.Try) for n in ast.walk(main_func))
            if not has_try_except:
                raise CodeValidationError(
                    "Function must include error handling (try/except)"
                )
    ...
    def _prepare_restricted_globals(
        self, step_dir: Path, permissions: Dict[str, List[str]]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for key, value in result.items():
                if not isinstance(value, (str, int, float, bool, list, dict)):
                    raise ExecutorError(
                        f"Unsupported type for output '{key}': {type(value)}"
                    )
        ...
        if step.tool == "generate_code":
            ...
            for field in required_fields:
                if field not in step.inputs:
                    raise ValueError(
                        f"Missing required field '{field}' for generate_code step"
                    )
            ...
            if "script_file" not in step.inputs and not hasattr(step, "code"):
                raise ValueError(
                    "Either script_file or code must be provided for execute_code step"
                )
    ...
    def _validate_step_dependencies(
        self, step: PlanStep, available_refs: Set[str]
    ) -> None:
            ...
            if "script_file" in step.inputs:
                ...
                if isinstance(script_ref, str) and script_ref.startswith("$"):
                    if script_ref not in available_refs:
                        raise ValueError(
                            f"Script file reference not available: {script_ref}"
                        )
...
7. 'outputs': Object mapping output names to types and descriptions"""
    ...
    def _execute_generate_code(
        ...
        try:
            ...
            return {"code_file": str(relative_path)}
    ...
    def _execute_code(
        ...
        script_path = resolved_inputs.get("code_file") or resolved_inputs.get(
            "script_file"
        )
        if not script_path:
        ...
        if not script_file:
            raise ExecutorError(
                f"Script file not found at any of: {[str(p) for p in script_paths]}"
            )
            ...
            if self.ecs_config.get("auto_promote_enabled", False):
                ...
                if eligible:
                    ...
                    for candidate_name in eligible:
                        candidate = self.candidate_manager.get_candidate_stats(
                            candidate_name
                        )
                        if candidate:
                            try:
                                ...
                                print(
                                    f"Auto-promotion failed for {candidate_name}: {str(e)}"
                                )
