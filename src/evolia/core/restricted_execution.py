...
def guarded_getitem(ob, index):
    ...
    if 'pandas' in sys.modules and isinstance(ob, (sys.modules['pandas'].DataFrame, sys.modules['pandas'].Series)):
        return ob[index]
...
def secure_open(filename, mode='r', *args, **kwargs):
    ...
    try:
        ...
        if not str(path).startswith(str(output_dir)):
            raise SecurityViolationError(f"Access to {path} outside output_dir is not allowed")
...
def restricted_import(name, globals=None, locals=None, fromlist=(), level=0, allowed_modules=None):
...
class RestrictedExecutor:
    ...
    def prepare_restricted_globals(self, inputs=None, output_dir=None, allowed_modules=None):
        """Prepare restricted globals for code execution."""
        logger.info("Adding allowed modules")
        
        if allowed_modules is None:
            allowed_modules = {'os', 'os.path', 'json', 'pandas', 'numpy', 'math', 'pathlib'}
        ...
        restricted_globals.update({
            ...
            '__import__': lambda name, globals=None, locals=None, fromlist=(), level=0: restricted_import(
                name, globals, locals, fromlist, level, allowed_modules=allowed_modules
            ),
        ...
        for module_name in allowed_modules:
            if module_name not in ('os', 'os.path'):  # Skip os since we handled it specially
                try:
    ...
    def execute_in_sandbox(self, script: str, inputs: Dict[str, Any], output_dir: str) -> Any:
