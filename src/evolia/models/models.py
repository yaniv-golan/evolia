    ...
    def from_step(cls, step: PlanStep, tool: "SystemTool") -> "SystemToolValidation":
        ...
        if not step.outputs:
            ...
            if output_type != tool.interface.return_type:
                validation.matches_interface = False
                validation.validation_errors.append(
                    f"Output type mismatch: expected {tool.interface.return_type}, got {output_type}"
                )
            ...
            "steps": [
                {
                    ...
                    "interface_validation": {
                        "matches_interface": step.interface_validation.matches_interface
                        if step.interface_validation
                        else False,
                        "validation_errors": step.interface_validation.validation_errors
                        if step.interface_validation
                        else [],
                    }
                    if step.interface_validation
...
class SystemTool:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "outputs": {
                name: {"type": output.type} for name, output in self.outputs.items()
            },
