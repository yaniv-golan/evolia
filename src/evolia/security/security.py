...
def validate_security_checks(config: Dict[str, Any]) -> Set[str]:
    ...
    security_checks = set(
        config.get("security_checks", AVAILABLE_SECURITY_CHECKS.keys())
    )
    ...
    if invalid_checks:
        logger.error(
            "Invalid security checks specified",
            extra={
                "payload": {
...
def get_rate_limiter(config: Dict[str, Any]) -> Optional["SubprocessRateLimiter"]:
    ...
    if rate_limit_config.get("enabled", False):
        ...
        logger.debug(
            ...
            extra={"payload": {**rate_limit_config, "component": "security"}},
        )
    else:
...
def get_subprocess_policy(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if policy_level not in valid_levels:
        logger.warning(
            f"Invalid policy level '{policy_level}', defaulting to 'none'",
            extra={
                "payload": {
...
def extract_command(node: ast.Call) -> Optional[str]:
    ...
    # Handle list literal
        ...
        for elt in first_arg.elts:
            if isinstance(elt, ast.Constant) and isinstance(elt.value, str):
                ...
                logger.warning(
                    "Non-string element in command list",
                    extra={
                        "payload": {
    ...
    # Handle f-strings
        ...
        logger.warning(
            "F-string in subprocess command",
            extra={
                "payload": {
                    "component": "security",
                    "operation": "command_extraction",
        ...
        if len(first_arg.args) == 1 and isinstance(first_arg.args[0], ast.List):
            ...
            for elt in first_arg.args[0].elts:
                if isinstance(elt, ast.Constant) and isinstance(elt.value, str):
                    ...
                    logger.warning(
                        "Non-string element in join arguments",
                        extra={
                            "payload": {
                                "element_type": type(elt).__name__,
                                "component": "security",
...
def validate_command(command: str) -> None:
    ...
    try:
        ...
        if any(char in base_cmd for char in shell_metacharacters):
            logger.warning(
                ...
                extra={
                    "payload": {
                        ...
                        "metacharacters": [
                            char for char in shell_metacharacters if char in base_cmd
                        ],
                        "component": "security",
            ...
            raise SecurityViolationError(
                f"Command '{base_cmd}' contains disallowed shell metacharacters"
            )
...
class SubprocessRateLimiter:
    ...
    def check_rate_limit(self) -> bool:
        ...
        if len(self.calls) >= self.max_calls:
            logger.warning(
                "Subprocess rate limit exceeded",
                extra={
                    "payload": {
                        ...
                        "time_until_next": (
                            min(self.calls) + self.period_seconds - current_time
                        )
                        if self.calls
                        else 0,
                    }
        ...
        logger.debug(
            "Subprocess call allowed by rate limiter",
            extra={
                "payload": {
                    "max_calls": self.max_calls,
                    "period_seconds": self.period_seconds,
...
class SecurityVisitor(ast.NodeVisitor):
    ...
    def visit_Import(self, node: ast.Import):
        ...
        for name in node.names:
            if name.name in self.config.get("blocked_modules", []):
                self._add_violation(
                    "imports", f"Import of module '{name.name}' is blocked"
                )
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if isinstance(node.func, ast.Name):
            ...
            if func_name in {"system", "popen"}:
                self._add_violation(
                    "system_calls", f"Direct {func_name}() call detected"
                )
            ...
            if isinstance(node.func.value, ast.Name):
                ...
                elif node.func.value.id == "os" and node.func.attr in {
                    "system",
                    "popen",
                }:
                    self._add_violation(
                        "system_calls", f"os.{node.func.attr}() call detected"
                    )
        ...
        if self._should_check("nested_functions"):
            ...
            for child in ast.walk(node):
                if isinstance(child, ast.FunctionDef) and child is not node:
                    self._add_violation(
                        "nested_functions", "Nested function definition detected"
                    )
                    break
    ...
    def visit_Nonlocal(self, node: ast.Nonlocal):
        ...
        for name in node.names:
            logger.warning(
                f"Nonlocal statement detected: {name}",
                extra={
                    "payload": {
                        "name": name,
                        "component": "security",
    ...
    def check_subprocess_call(self, node: ast.Call, command: str) -> None:
        ...
        for kw in node.keywords:
            if (
                ...
                logger.warning(
                    "Subprocess call denied: shell execution not allowed",
                    extra={
                        "payload": {
                            "command": command,
                            "component": "security",
        ...
        if self.subprocess_policy["level"] == "none":
            logger.warning(
                "Subprocess call denied: policy level none",
                extra={
                    "payload": {
                        "command": command,
                        "component": "security",
            ...
            self._add_violation(
                "subprocess", "Subprocess calls are not allowed under default policy"
            )
            return
        ...
        if base_command in self.subprocess_policy.get("blocked_commands", []):
            logger.warning(
                "Subprocess call denied: blocked command",
                extra={
                    "payload": {
                        "command": command,
        ...
        if self.subprocess_policy["level"] == "system_tool":
            if not self.invoked_by_tool:
                logger.warning(
                    "Subprocess call denied: system tool policy",
                    extra={
                        "payload": {
                            "command": command,
                            "base_command": base_command,
            ...
            if base_command not in self.subprocess_policy.get("allowed_commands", []):
                logger.warning(
                    "Subprocess call denied: not an allowed system tool",
                    extra={
                        "payload": {
                            "command": command,
                            "base_command": base_command,
            ...
            if allowed_commands and base_command not in allowed_commands:
                logger.warning(
                    "Subprocess call denied: not in allowed list",
                    extra={
                        "payload": {
                            "command": command,
                            "base_command": base_command,
        ...
        if self.rate_limiter and not self.rate_limiter.check_rate_limit():
            logger.warning(
                "Subprocess call denied: rate limit exceeded",
                extra={
                    "payload": {
                        "command": command,
                        "component": "security",
        ...
        logger.debug(
            f"Subprocess call allowed: {command}",
            extra={
                "payload": {
                    "command": command,
...
def validate_code_security(code: str, config: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    try:
        ...
        raise SecurityViolationError(
            f"Syntax error in code: {str(e)}", code, {"error": str(e)}
        )
...
if __name__ == "__main__":
    ...
    config = {
        "subprocess_policy": {
            ...
            "rate_limit": {"enabled": True, "max_calls": 10, "period_seconds": 60},
        }
    }
