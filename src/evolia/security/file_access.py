...
def extract_paths(task_description: str) -> List[str]:
    ...
    for token in tokens:
        ...
        try:
            ...
            logger.warning("Invalid path in task description: {path}", extra={
                'payload': {
                    'path': path,
...
def _check_path_exists(path: Union[str, Path], log_warning: bool = True) -> bool:
    ...
    if not exists and log_warning:
        logger.warning(f"Path does not exist: {path}", extra={
            'payload': {
                'path': path,
                'component': 'file_access',
...
def get_allowed_paths(config_permissions: List[Dict[str, Any]], cli_permissions: Dict[str, List[str]]) -> Dict[str, Any]:
    ...
    for path_config in config_permissions:
        ...
        try:
            ...
            if recursive:
                ...
                paths_to_add.extend([d.resolve() for d in resolved_path.rglob('*') if d.is_dir()])
            else:
                paths_to_add = [resolved_path]
            ...
            logger.warning(f"Invalid path in config permissions: {path}", extra={
                'payload': {
                    'path': path,
                    'component': 'file_access',
        ...
        for perm_type in ['read', 'write', 'create', 'create_only']:
            ...
            for path in paths:
                try:
                    ...
                    logger.warning(f"Invalid path in CLI permissions: {path}", extra={
                        'payload': {
                            'path': path,
                            'component': 'file_access',
    ...
    logger.debug("Resolved file access permissions", extra={
        'payload': {
...
def _check_read_permission(file_path: Path, permissions: Dict[str, List[str]], default_policy: str = "deny") -> None:
    ...
    if not is_allowed and default_policy == "allow":
        is_allowed = True
        logger.debug(f"Path {file_path} allowed by default policy", extra={
            'payload': {
                'path': str(file_path),
                'component': 'file_access',
...
def _check_write_permission(file_path: Path, permissions: Dict[str, List[str]], ephemeral_dir: Optional[str] = None) -> None:
    ...
    if file_exists:
        ...
        if not is_allowed:
            if ephemeral_dir:
                logger.warning("Write attempt outside ephemeral directory", extra={
                    'payload': {
                        'path': str(file_path),
        ...
        if not is_allowed:
            if ephemeral_dir:
                logger.warning("Create attempt outside allowed paths", extra={
                    'payload': {
                        'path': str(file_path),
                        'ephemeral_dir': ephemeral_dir,
                        'component': 'file_access',
...
def _check_create_permission(file_path: Path, permissions: Dict[str, List[str]], ephemeral_dir: Optional[str] = None) -> None:
    ...
    if not is_allowed:
        if ephemeral_dir:
            logger.warning("Create attempt outside ephemeral directory", extra={
                'payload': {
                    'path': str(file_path),
                    'ephemeral_dir': ephemeral_dir,
                    'component': 'file_access',
...
def validate_path(file_path: Union[str, Path], allowed_paths: Optional[List[str]] = None,
                mode: str = 'r', ephemeral_dir: Optional[str] = None,
                permissions: Optional[Dict[str, List[str]]] = None,
                default_policy: str = 'deny') -> Path:
    ...
    if ephemeral_dir:
        try:
            ...
            raise FileAccessViolationError(f"Invalid ephemeral directory: {ephemeral_dir}")
    ...
    if is_write and 'create_only' in permissions:
        ...
        if is_creation_only and resolved_file_path.exists():
            raise FileAccessViolationError(f"Directory is creation-only, cannot modify existing files: {file_path}")
    ...
    if is_write and ephemeral_dir:
        if str(resolved_file_path).startswith(str(ephemeral_path)) and resolved_file_path.exists():
            ...
            if not has_write_permission:
                raise FileAccessViolationError("Cannot modify existing file in ephemeral directory")
            
    return resolved_file_path

def restricted_open(file_path: str, mode: str = 'r', allowed_paths: Optional[List[str]] = None,
                ephemeral_dir: Optional[str] = None, permissions: Optional[Dict[str, List[str]]] = None,
                default_policy: str = 'deny') -> IO:
    ...
    if not re.match(r'^[rwa]\+?$', mode):
        raise FileAccessViolationError(f"Invalid mode: {mode}. Only r, w, a modes are allowed")
    
    # Check if write operation without ephemeral directory
    if ('w' in mode or 'a' in mode or '+' in mode) and not ephemeral_dir:
        logger.warning("Write attempt without ephemeral directory specified", extra={
            'payload': {
                'path': str(file_path),
                'mode': mode,
                'component': 'file_access',
...
def get_safe_open(allowed_paths: List[str] = None, ephemeral_dir: str = None, permissions: Dict[str, List[str]] = None, default_policy: str = "deny"):
    """
    ...
    """
    logger = logging.getLogger('evolia')
    logger.debug("Creating safe_open function", extra={
        'payload': {
