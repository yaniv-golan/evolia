...
class ModelJSONEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        if isinstance(obj, Parameter):
            ...
            return {"type": obj.type, "description": getattr(obj, "description", None)}
    ...
    def filter(self, record):
        ...
        if not self.verbose:
            return (
                ...
                and record.levelno >= logging.INFO  # No structured logging
                and not record.name.startswith(  # Only INFO and above
                    "evolia."
                )  # No internal module logs
            )
        return True
