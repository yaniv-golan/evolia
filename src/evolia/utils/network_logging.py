...
class RequestsClient:
    ...
    def __init__(
        ...
        logger.debug(
            f"Rate limiting enabled: {self.rate_limit} calls per {self.rate_window}s"
        )
    ...
    def _check_rate_limit(self):
        ...
        if len(self.calls) >= self.rate_limit:
            logger.error(
                f"Rate limit exceeded: {self.rate_limit} calls per {self.rate_window}s"
            )
...
class LoggedRequests:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.rate_limit.get("enabled", False):
            ...
            logger.debug(
                f"Rate limiting enabled: {self.max_calls} calls per {self.period}s"
            )
